    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <span class="badge bg-warning">PANEN PARSIAL</span>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('updateProgress', ['id' => $todo->_id]) }}">
                    @csrf
                    <input type="hidden" name="id_panen" value="{{ $todo->id_panen }}">
                    <input type="hidden" name="id_todo_parsial" value="{{ $todo->id_todo_parsial }}">
                    <div class="row">
                        ...
                        @foreach ($accordionItems as $index => $accordionItem)
                            @include('accordion-item', [
                                ...
                                'checked' => $listTodoParsial->where('id_todo_parsial', $accordionItem->_id)->first() ? 'checked' : '',
                                'todoParsialId' => $accordionItem->_id,
                            ])
                        @endforeach
                        </div>
                        </div>
