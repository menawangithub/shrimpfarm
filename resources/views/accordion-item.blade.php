<label class="list-group-item d-flex align-items-center">
    @php
        $todo = \App\Models\ListTodoParsialModel::where('id_todo_parsial', $todoParsialId)
            ->where('id_panen', $todo->id_panen)
            ->first();
        $isChecked = $todo && $todo->checked === 'on';
    @endphp

    <input class="form-check-input me-4" type="checkbox" name="checkboxes[{{ $todoParsialId }}]" {{ $isChecked ? 'checked' : '' }}>
