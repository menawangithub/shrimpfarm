    ...
    <div class="container-xxl flex-grow-1 container-p-y">
      ...
      <nav aria-label="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="/kontenedukasi">Konten Budidaya Video</a></li>
          <li class="breadcrumb-item active" aria-current="page">{{$data->judul_artikel}}</li>
        </ol>
      </nav>

      <!-- Judul Artikel -->
       <h2 class="fw-bold py-3 text-primary">{{$data->judul_artikel}}</h2>
      ...
      <!-- Video Player -->
      <div class="embed-responsive-16by9 d-flex justify-content-center align-items-center">
          <img src="{{ $data->gambar }}" alt="Artikel Foto" style="width: 50%; height: 50%;">
      </div>

      <br>
      <!-- Isi Artikel -->
      <div class="mt-4" style="margin-left: 50px; margin-right: 50px;">
          {!! nl2br(e($data->isi_artikel)) !!}
      </div>
      <!-- Bagian Komentar -->
      <h5 class="mt-0 mb-1">{{ $comment->user['nama_user'] ?? '' }}</h5>
      <div class="mt-4">
          <h3 class="fw-bold py-3 text-primary">Komentar</h3>
          <form method="post" action="{{ route('PostCommentArticle')}}">
            @csrf
              <input type="hidden" name="id_article" value="{{ $data->_id }}">
